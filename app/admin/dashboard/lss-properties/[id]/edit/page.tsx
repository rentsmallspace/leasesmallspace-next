...
import { getPropertyById, updateProperty, Property, UpdatePropertyData } from '@/lib/admin/properties';
import ImageUpload from '@/components/ImageUpload';
import { StorageUploadResult, getStoragePublicUrl } from '@/lib/storage';
...
export default function EditPropertyPage() {
  ...
  const [currentImages, setCurrentImages] = useState<StorageUploadResult[]>([]);
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        ...
        // Convert existing images to StorageUploadResult format (path + publicUrl)
        const existingImagesData: StorageUploadResult[] = [];
        if (result.data.images && result.data.images.length > 0) {
          result.data.images.forEach((path: string) => {
            existingImagesData.push({
              path,
              publicUrl: getStoragePublicUrl(path),
              format: path.split('.').pop() || 'jpg',
            });
          });
        }
        setCurrentImages(existingImagesData);

        // Ensure images are properly set in form data
        const imageIds = result.data.images || [];
        const primaryImage = result.data.primary_image || (imageIds.length > 0 ? imageIds[0] : '');
        const additionalImages = result.data.additional_images || imageIds.slice(1);
        ...
        setFormData({
          ...
          deal_score: result.data.deal_score || '',
          is_active: result.data.is_active ?? true,
          clear_height: result.data.clear_height || undefined,
          loading_doors: result.data.loading_doors || undefined,
          power_service: result.data.power_service || '',
          flooring_type: result.data.flooring_type || '',
          lighting_type: result.data.lighting_type || '',
          hvac_type: result.data.hvac_type || '',
          parking_spaces: result.data.parking_spaces || undefined,
          outdoor_storage: result.data.outdoor_storage || false,
          neighborhood: result.data.neighborhood || '',
          nearby_highways: result.data.nearby_highways || [],
          distance_to_downtown: result.data.distance_to_downtown || undefined,
          public_transit: result.data.public_transit || false,
          lease_term: result.data.lease_term || '',
          deposit_requirements: result.data.deposit_requirements || '',
          utilities_responsibility: result.data.utilities_responsibility || '',
          maintenance_responsibility: result.data.maintenance_responsibility || '',
          insurance_requirements: result.data.insurance_requirements || '',
  ...
  // Simple image handling - no complex state management
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.title || !formData.address || !formData.city || !formData.property_type || !formData.size_sqft || formData.size_sqft <= 0 || !formData.price_monthly || formData.price_monthly <= 0) {
        throw new Error('Please fill in all required fields');
      }

      // Persist current image list from state (so deletions are never lost)
      const imagePaths = currentImages.map((img) => img.path);
      const primaryImage = imagePaths.length > 0 ? imagePaths[0] : '';
      const additionalImages = imagePaths.slice(1);
      const payload: UpdatePropertyData = {
        ...formData,
        images: imagePaths,
        primary_image: primaryImage,
        additional_images: additionalImages,
      ...
      const result = await updateProperty(payload);
      ...
      router.push(`/admin/dashboard/lss-properties/${propertyId}`);
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">
              Update property information in the LSS database.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link
              href={`/admin/dashboard/lss-properties/${propertyId}`}
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 sm:p-8">
          <div className="mb-6">
            <h2 className="text-base font-semibold text-gray-900 mb-5 pb-3 border-b border-gray-100">Property Images</h2>
            <p className="text-sm text-gray-600 mt-1">
              Upload high-quality images of your property. The first image will be used as the primary image for listings.
            </p>
          </div>
